...
public:
    ...
    bool deferredLoadingComplete() const {
        return lazyLoadStep_ >= 4 + static_cast<uint8_t>(Expression::Count);
    }
...
private:
    static constexpr uint8_t kMaxBlinkFrames = 5;
    ...
    SpriteLayer blinkHalfSprite_;
    SpriteLayer blinkQuarterSprite_;
    SpriteLayer blinkFrames_[kMaxBlinkFrames];
    uint16_t blinkFrameDur_[kMaxBlinkFrames];
    uint8_t blinkFrameCount_;
    uint8_t blinkFrameIdx_;
