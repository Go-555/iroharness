...
.iroharness/
  agent-bank/
    _index.md
    seed-manifest.json          # ORIGIN AUTHORITY: ids seeded from builtins;
                                # anything not listed is "minted" (fail-safe)
    verification-ledger.json    # SANDBOX AUTHORITY: recorded trial outcomes
                                # (runSandboxVerification); the promotion gate
                                # reads this, not the caller's claim
...
Both root-level JSON files live **outside every recipe folder** on purpose: a
recipe's own frontmatter can never taint them (B-1 / B-2).
...
- **Sandbox verification is recorded, not claimed**
  (`src/agent-bank/sandbox.js`): the isolated trial's outcome is written to
  `verification-ledger.json` at the bank root, and when the gate is given the
  bank root it derives its sandbox condition from that record — a recorded
  failure defeats a self-reported pass (the CLI always passes the root).
- **"Keep" decided by reuse value (累積), not a single lucky success.**
- Promotion in tool terms = writing a persistent definition:
  OpenClaw `agents add --json`; Claude Code `~/.claude/agents/<id>.md`. That
  write goes through `persistRecipe` (`src/agent-bank/persist-guard.js`): only
  **active** recipes, only into the **scoped workspace** by default — host-global
  agent dirs (`~/.claude/agents/` etc.) sit on a deny list and open only with
  explicit owner approval (B-4).
