  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) => {
      ...
      if (typeof item === "string" && /[?&]token=/i.test(item)) {
        return [key, item.replace(/([?&]token=)[^&]+/i, "$1[redacted]")];
      }
...
const toWebSocketUrl = (baseUrl, path) =>
  `${String(baseUrl).replace(/^http:/, "ws:").replace(/^https:/, "wss:")}${path}`;

const toStackChanFirmwareWebSocketConfig = ({ baseUrl, path, token }) => {
  const url = new URL(toWebSocketUrl(baseUrl, path));
  const defaultPort = url.protocol === "wss:" ? 443 : 80;
  return {
    ws_host: url.hostname,
    ws_port: Number(url.port || defaultPort),
    ws_path: `${url.pathname}?token=${encodeURIComponent(token)}`
  };
};

const createStackChanProvisioningMarkdown = ({ deviceId, baseUrl, realtimeUrl }) => `# StackChan Provisioning

This file is generated by \`iroharness connect stackchan\`.

## Device

- Device ID: \`${deviceId}\`
- Host URL: \`${baseUrl}\`
- Face polling: \`${baseUrl}/stackchan/face\`
- Device invoke: \`${baseUrl}/device/stackchan/invoke\`
- Realtime WebSocket: \`${realtimeUrl}\`
- Body events: \`${baseUrl}/body/${deviceId}/events\`

## First Flash

1. Start from the AIAvatarStackChan StackChan runtime shape.
2. Copy the generated AIAvatarStackChan-style values from
   \`.iroharness/connections/stackchan-firmware-config.json\` into the device
   \`/config.json\`.
3. Build and upload the PlatformIO project to the M5Stack/CoreS3 device.
4. Start the Mac mini host with \`npm run example:slack-stackchan\`.
5. Confirm the device can open the realtime WebSocket at \`${realtimeUrl}\`.

## Runtime Updates

- Character, brain, permission, and Project OS updates stay on the Mac mini host.
- Wi-Fi, display, audio buffer, device token, and host URL changes require updating the firmware config.
- Today the firmware path is manual flash/provisioning.
- OTA should be added in the firmware package or device relay, not inside the macro harness core.
- The 1-second conversation path should use the realtime WebSocket.

## Security Boundary

The device token is required through the WebSocket query token, \`Authorization:
Bearer ...\`, or \`x-iroharness-device-token\`.
Do not commit \`.iroharness/connections/stackchan-firmware-config.json\`; it contains Wi-Fi and device token values.
`;
...
const connectStackChan = (args) => {
  ...
  const realtimePath = `/device/${deviceId}/realtime`;
  const realtimeUrl = toWebSocketUrl(baseUrl, realtimePath);
  const firmwareWebSocket = toStackChanFirmwareWebSocketConfig({
    baseUrl,
    path: realtimePath,
    token: deviceToken
  });
  ...
  const deviceConfig = {
    ...
    metadata: {
      connectionMode: "aiavatarstackchan-websocket",
      auth: "websocket-query-or-device-token-header",
      provisioning: "manual-flash-now-ota-firmware-package-later",
      deviceReachability
    }
  };
  const firmwareConfig = {
    ...
    ptt_max_seconds: 12,
    ptt_min_seconds: 1,
    ptt_hold_threshold_ms: 500,
    nade_invoke_prompt: "$頭を撫でられました。短く反応してください。",
    vision_invoke_prompt: "$見えているものに反応してください。",
    iroharness: {
      base_url: baseUrl,
      invoke_url: `${baseUrl}/device/stackchan/invoke`,
      body_events_url: `${baseUrl}/body/${deviceId}/events`,
