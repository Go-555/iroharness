...
1. Open \`examples/stackchan-face-poller/data/config.json\`.
2. Copy values from \`.iroharness/connections/stackchan-firmware-config.json\`.
...
5. Confirm the device can reach \`${baseUrl}/stackchan/face\`.

## Runtime Updates

- Character, brain, permission, and Project OS updates stay on the Mac mini host.
- Wi-Fi, display, retry, device token, and host URL changes require updating the firmware config.
- Today the included firmware path is manual flash/provisioning.
- OTA should be added in the firmware package or device relay, not inside the macro harness core.
- The 1-second conversation path should use the realtime WebSocket, not face polling.

## Security Boundary

The device token is required through \`x-iroharness-device-token\`.
...
const connectStackChan = (args) => {
  ...
  const deviceConfig = {
    ...
    metadata: {
      ...
      realtimeConnectionMode: "websocket",
