...
const createSlackCodexCompanion = () => {
  ...
  const server = createServer(async (request, response) => {
    if (request.method === "GET" && request.url === "/health") {
      ...
      response.end(JSON.stringify({ ok: true, service: "iroharness-slack-codex" }));
      return;
    }
    ...
    if (!verifySlackSignature({ body, headers: request.headers, signingSecret })) {
      ...
      response.end(JSON.stringify({ ok: false, error: "invalid_slack_signature" }));
      return;
    }
    ...
    listen() {
      return new Promise((resolve) => {
        server.listen(port, host, () => {
          resolve({ url: `http://${host}:${port}/slack/events` });
        });
      });
    },
    close() {
